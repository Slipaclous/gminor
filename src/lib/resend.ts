...
export async function sendContactNotificationEmail({
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      to: [recipientEmail],
      replyTo: email,
      subject: `🎯 Nouvelle demande de devis : ${service || "Contact"} - ${name}`,
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #0b0c10; color: #ffffff; padding: 40px 20px;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #13141c; border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 32px;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
              <tr>
                <td style="padding: 8px 0; color: #a1a1aa; font-size: 13px; width: 150px;">Nom du prospect :</td>
              ...
              ${contactPref ? `
              <tr>
                <td style="padding: 8px 0; color: #a1a1aa; font-size: 13px;">Canal préféré :</td>
                <td style="padding: 8px 0; color: #ffffff; font-size: 14px; font-weight: 600;">${contactPref}</td>
              </tr>` : ""}
              ${availabilityPref ? `
              <tr>
                <td style="padding: 8px 0; color: #a1a1aa; font-size: 13px;">Créneau / Disponibilité :</td>
                <td style="padding: 8px 0; color: #ffffff; font-size: 14px; font-weight: 600;">${availabilityPref}</td>
              </tr>` : ""}
            </table>
            ...
            <div style="background-color: #070709; border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 20px; margin-bottom: 24px;">
              <span style="display: block; color: #71717a; font-size: 11px; text-transform: uppercase; font-weight: 700; margin-bottom: 8px;">Détails du message :</span>
