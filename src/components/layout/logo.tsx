...
export function Logo({ className = "", showSubtitle = false }: LogoProps) {
  return (
    ...
    >
      {/* Option C: Emblème Signature Terminal Craft (Carré noir mat avec GM épuré & diode émeraude) */}
      <div className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-[#0d0e12] border border-white/[0.12] group-hover:border-emerald-500/40 flex items-center justify-center transition-all duration-300 shadow-md group-hover:shadow-[0_0_20px_rgba(16,185,129,0.18)] shrink-0 overflow-hidden">
        ...
        {/* Typographic GM monogram (Clean, minimal, ultra-legible) */}
        <span className="text-xs sm:text-sm font-black font-mono tracking-tight text-white group-hover:text-emerald-300 transition-colors">
          GM
        </span>

        {/* Micro-diode émeraude pulsante (statut actif) */}
      ...
      {/* Option B: Mot-Symbole Typographique Studio (gminor. ou gauthier minor) */}
      <div className="flex flex-col">
        <div className="flex items-center text-base sm:text-lg font-bold tracking-tight text-white font-sans leading-none">
          <span>gminor</span>
