    ...
    return (
      <div className="group relative rounded-3xl bg-[#0d0d10] border border-white/[0.08] hover:border-emerald-500/40 transition-all duration-300 overflow-hidden shadow-2xl p-5 sm:p-8 lg:p-10 hover:shadow-[0_0_35px_rgba(16,185,129,0.07)]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-12 items-center relative z-10">
          ...
          <div className="lg:col-span-7 space-y-5 sm:space-y-6 flex flex-col justify-between order-1">
            <div className="space-y-3 sm:space-y-4">
              {/* Category Badge + Featured Indicator */}
              <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                <span className="px-2.5 py-1 rounded-lg text-[11px] sm:text-xs font-mono uppercase tracking-wider font-bold bg-emerald-950/80 text-emerald-400 border border-emerald-500/30 flex items-center gap-1.5 shadow-sm">
                  <Sparkles className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                  <span>Projet à la une</span>
                </span>
                <span className="px-2.5 py-1 rounded-lg text-[11px] sm:text-xs font-mono uppercase tracking-wider font-semibold bg-zinc-900 text-zinc-300 border border-white/[0.08]">
              ...
              {/* 3 Impact Metrics Grid (Mobile optimized) */}
              {project.metrics && project.metrics.length > 0 && (
                <div className="grid grid-cols-3 gap-2 sm:gap-3 pt-1 sm:pt-2">
                  {project.metrics.slice(0, 3).map((m, idx) => (
                    <div
                      key={idx}
                      className="p-2.5 sm:p-3.5 rounded-xl sm:rounded-2xl bg-black/60 border border-white/[0.08] space-y-0.5 sm:space-y-1"
                    >
                      <span className="text-[10px] sm:text-[11px] text-zinc-400 block font-mono truncate">
                        {m.label}
                      </span>
                      <span className="text-xs sm:text-base lg:text-lg font-extrabold text-white font-mono tracking-tight block">
                        {m.value}
                      </span>
                    </div>
                  ))}
          ...
          <div className="lg:col-span-5 w-full order-2">
            <div className="rounded-2xl bg-black/90 border border-white/[0.1] overflow-hidden shadow-2xl relative">
              ...
              <div className="px-3 sm:px-4 py-2.5 sm:py-3 bg-[#13141a] border-b border-white/[0.08] flex items-center justify-between">
              ...
              <div className="relative w-full h-48 sm:h-64 lg:h-80 bg-[#08080a] overflow-hidden flex items-center justify-center p-3">
                {project.imageUrl ? (
                  <>
                    <Image
