...
export function ProjectCard({
    ...
    return (
      <div className="group relative rounded-3xl bg-[#0d0d10] border border-white/[0.08] hover:border-emerald-500/40 transition-all duration-300 overflow-hidden shadow-2xl p-5 sm:p-8 lg:p-10 hover:shadow-[0_0_35px_rgba(16,185,129,0.07)]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-8 lg:gap-12 items-center relative z-10">
          ...
          <div className="lg:col-span-7 space-y-4 sm:space-y-6 flex flex-col justify-between order-1">
            <div className="space-y-3 sm:space-y-4">
              {/* Category Badge + Featured Indicator */}
              <div className="flex flex-wrap items-center gap-2">
                <span className="px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-mono uppercase tracking-wider font-bold bg-emerald-950/80 text-emerald-400 border border-emerald-500/30 flex items-center gap-1.5 shadow-sm">
                  <Sparkles className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                  <span>Projet à la une</span>
                </span>
                <span className="px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-mono uppercase tracking-wider font-semibold bg-zinc-900 text-zinc-300 border border-white/[0.08]">
              ...
              <div className="space-y-1.5 sm:space-y-2">
                <h3 className="text-xl sm:text-3xl lg:text-4xl font-extrabold text-white tracking-tight">
                  <Link
                    ...
                    className="hover:underline flex items-center gap-2 sm:gap-3 group/link"
                  >
                    <span>{project.title}</span>
                    <ArrowUpRight className="w-5 h-5 sm:w-6 sm:h-6 text-zinc-500 group-hover/link:text-emerald-400 group-hover/link:translate-x-1 group-hover/link:-translate-y-1 transition-all" />
                  </Link>
                </h3>
                <p className="text-xs sm:text-base text-zinc-300 leading-relaxed max-w-xl font-normal line-clamp-2 sm:line-clamp-none">
                  {project.tagline}
                </p>
              </div>

              {/* 3 Impact Metrics Grid (Desktop only - Hidden on mobile to avoid clutter) */}
                ...
                <div className="hidden sm:grid sm:grid-cols-3 gap-3 pt-2">
                  {project.metrics.slice(0, 3).map((m, idx) => (
                    ...
                    >
                      ...
                      <span className="text-base lg:text-lg font-extrabold text-white font-mono tracking-tight block">
                        {m.value}
                      </span>
                    </div>
                  ))}
              ...
              {/* Tech Stack Badges (Filtered: 3 on mobile, full on desktop) */}
              <div className="flex flex-wrap gap-1.5 pt-1">
                {project.techStack.slice(0, 3).map((tech) => (
                  <span
                    key={tech}
                    className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md text-[10px] sm:text-xs font-mono bg-zinc-900/90 text-zinc-300 border border-white/[0.08]"
                  >
                    {tech}
                  </span>
                ))}
                {/* Extra pills visible only on desktop */}
                {project.techStack.slice(3).map((tech) => (
                  <span
                    key={tech}
                    className="hidden sm:inline-block px-2.5 py-1 rounded-md text-xs font-mono bg-zinc-900/90 text-zinc-300 border border-white/[0.08]"
                  >
                    {tech}
                  </span>
                ))}
          ...
          <div className="lg:col-span-5 w-full order-2">
            <div className="rounded-2xl bg-black/90 border border-white/[0.1] overflow-hidden shadow-2xl relative">
              ...
              <div className="px-3 sm:px-4 py-2 sm:py-3 bg-[#13141a] border-b border-white/[0.08] flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <span className="w-2 sm:w-2.5 h-2 sm:h-2.5 rounded-full bg-zinc-700 inline-block" />
                  <span className="w-2 sm:w-2.5 h-2 sm:h-2.5 rounded-full bg-zinc-700 inline-block" />
                  <span className="w-2 sm:w-2.5 h-2 sm:h-2.5 rounded-full bg-zinc-700 inline-block" />
                </div>
                <span className="text-[10px] sm:text-[11px] font-mono text-zinc-400 truncate max-w-[160px] sm:max-w-[200px]">
              ...
              <div className="relative w-full h-44 sm:h-64 lg:h-80 bg-[#08080a] overflow-hidden flex items-center justify-center p-2.5 sm:p-4">
                {project.imageUrl ? (
                  <>
                    <Image
                      ...
                      alt={project.title}
                      fill
                      className={`object-contain p-3 sm:p-4 opacity-85 group-hover:opacity-100 transition-opacity duration-500 ${
                        isSvg
  ...
  return (
    <div className="group relative rounded-3xl bg-[#0d0d10] border border-white/[0.08] hover:border-white/[0.22] hover:border-emerald-500/30 transition-all duration-300 flex flex-col justify-between overflow-hidden shadow-xl p-5 sm:p-7 hover:shadow-[0_0_24px_rgba(16,185,129,0.04)]">
      {/* Subtle top border beam highlight on hover */}
      ...
      <div className="space-y-4 sm:space-y-5">
        ...
        <div className="rounded-2xl bg-black/90 border border-white/[0.08] overflow-hidden relative">
          ...
          <div className="relative w-full h-40 sm:h-48 bg-[#08080a] overflow-hidden flex items-center justify-center p-2">
            {project.imageUrl ? (
              <>
                <Image
                  src={project.imageUrl}
        ...
        <div className="flex items-center justify-between gap-4">
          <span className="px-2.5 py-1 rounded-md text-[10px] sm:text-[11px] font-mono uppercase tracking-wider font-semibold bg-zinc-900 text-zinc-300 border border-white/[0.08]">
            {project.categoryLabel}
          </span>
        ...
        {/* Key Metrics (Desktop only - Hidden on mobile) */}
        {project.metrics && project.metrics.length > 0 && (
          <div className="hidden sm:grid sm:grid-cols-2 gap-2 p-3 rounded-xl bg-black/80 border border-white/[0.06]">
        ...
        {/* Tech Stack Pills (2 on mobile, full on desktop) */}
        <div className="flex flex-wrap gap-1.5 pt-1">
          {project.techStack.slice(0, 2).map((tech) => (
            <span
              key={tech}
              className="px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-mono text-zinc-400 bg-zinc-900 border border-white/[0.05]"
            >
              {tech}
            </span>
          ))}
          {project.techStack.slice(2, 4).map((tech) => (
            <span
              key={tech}
              className="hidden sm:inline-block px-2 py-0.5 rounded-md text-[11px] font-mono text-zinc-400 bg-zinc-900 border border-white/[0.05]"
            >
              {tech}
            </span>
          ))}
          {project.techStack.length > 2 && (
            <span className="sm:hidden px-1.5 py-0.5 rounded-md text-[10px] font-mono text-zinc-500">
              +{project.techStack.length - 2}
            </span>
          )}
        </div>
      ...
      {/* Bottom Actions */}
      <div className="pt-4 sm:pt-5 mt-4 sm:mt-5 border-t border-white/[0.08] flex items-center justify-between gap-3 text-xs">
        <Link
          href={`/projets/${project.slug}`}
          className="font-bold text-white hover:text-emerald-400 flex items-center gap-1 transition-colors text-xs"
        >
        ...
        {project.liveUrl && (
          <a
            ...
            className="text-zinc-400 hover:text-white flex items-center gap-1 transition-colors font-mono text-[10px] sm:text-[11px]"
          >
            <span>Démo Live</span>
